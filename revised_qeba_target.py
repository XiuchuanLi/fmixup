...
def filter(arr, flt):
    ...
    farr = np.fft.fftshift(farr)
    farr = farr * flt
    farr = np.fft.ifftshift(farr)
    iarr = np.fft.ifft2(farr)
    iarr = iarr.real.astype(np.float32)
    return torch.tensor(iarr)


def fbinary_search(x_ori, x_adv, label_tar, Distance, r_low=0.0, r_high=1.42):
    ...
    if norm_model(x_candidate) != label_tar:
        return x_adv
    return x_candidate
...
for sample_id, (x_ori, _) in enumerate(clean_examples):
    ...
    x_adv = fbinary_search(x_ori, x_adv, label_tar, Distance)
    query += 10
    ...
    while query <= args.max_query:
        ...
        x_sadv, dist_post, count = binary_search(x_ori, x_adv, label_tar, Distance)
        query += count
        if orthogonal % 10 == 0:
            x_fadv = fbinary_search(x_ori, x_sadv, label_tar, Distance)
            query += 10
            if torch.norm(x_sadv - x_ori) > 0.5 + torch.norm(x_fadv - x_ori):
                print(f'FFT work {orthogonal}')
                x_sadv, _, count = binary_search(x_ori, x_fadv, label_tar, Distance)
                query += count
        x_adv = x_sadv

    LOGGER.info(f"{sample_id}, {query} queries")
    with open(f'revised_qeba_target_{args.model}.csv', 'a', newline='') as csvfile:
