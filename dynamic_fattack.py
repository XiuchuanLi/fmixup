...
num_clean, num_reference = 500, args.max_query // 5
...
for i in range(num_clean):
    ...
    perturb = torch.zeros([num_reference * 5] + list(img.shape))
    for r_idx, radius in enumerate([1.0, 0.75, 0.6, 0.5, 0.43]):
        r_l_left, r_l_right = 0, radius
        # calculate r_l via binary search in [0, r_h]
        while r_l_right - r_l_left > 1e-3:
            r_l_mid = (r_l_left + r_l_right) / 2
            bp_filter = band_pass_filter(r_l_mid, radius)
            ...
            if torch.norm(pass_clean) < args.threshold * 0.6:
                r_l_right = r_l_mid
            else:
                r_l_left = r_l_mid

        r_l, r_h = r_l_right, radius
        ...
        for j in range(num_reference):
            ...
            perturb[num_reference * r_idx + j] = calculate_perturb(pass_clean, pass_reference_j, args.threshold)
    ...
    # attack succeeds
        ...
        if args.bl:
            log_file = f'dynamic_fattack_bl_{args.model}_{args.threshold}.csv'
            ...
            log_file = f'dynamic_fattack_bd_{args.model}_{args.threshold}.csv'
            ...
            log_file = f'dynamic_fattack_{args.model}_{args.threshold}.csv'
            LOGGER.info(f"{i}, Succeed, {index_adv + 1} queries")
            write_log(log_file, [i, index_adv + 1, torch.norm(adv_img[index_adv] - img).item()])
